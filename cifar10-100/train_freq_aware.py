...
parser.add_argument('--no-l1-sparsity', action='store_true', default=False,
                    help='Disable Level 1 coefficient sparsity')
parser.add_argument('--no-l2-sparsity', action='store_true', default=False,
                    help='Disable Level 2 channel sparsity')
parser.add_argument('--no-compensate-gate', action='store_true', default=False,
                    help='Disable frequency compensation gate in MLP')
...
def main():
    ...
    if args.model == 'swformer_freq_aware':
        ...
        model = create_model(
            ...
            use_l1_sparsity=not args.no_l1_sparsity,
            use_l2_sparsity=not args.no_l2_sparsity,
            use_compensate_gate=not args.no_compensate_gate,
        )
    ...
    if args.local_rank == 0:
        print("\n" + "="*60)
        print("Model created. Energy calculator will be used after each epoch.")
        print("="*60 + "\n")
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    has_stats_method = hasattr(model, 'get_model_stats')
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if last_batch and has_stats_method:
                val_stats = model.get_model_stats()

                if args.local_rank == 0 and val_stats:
                    _logger.info(f'[DEBUG] Collected stats in validate: {list(val_stats.keys())}')
