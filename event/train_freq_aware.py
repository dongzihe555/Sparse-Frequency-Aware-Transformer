...
from spikingjelly.datasets import cifar10_dvs, integrate_events_by_fixed_frames_number
...
from dataset_tonic import load_data_tonic
import numpy as np
import math
...
parser.add_argument('-data-dir', '--data-dir', '--data_path', '--data-path', dest='data_dir', metavar='DIR', default="", help='path to dataset')
parser.add_argument('--use-tonic', action='store_true', default=False, help='use tonic backend for event datasets')
parser.add_argument('--use-nda', action='store_true', default=False, help='enable Neuromorphic Data Augmentation (NDA) for event datasets (default: False)')
parser.add_argument('--T', default=None, type=int, help='simulation steps for event data (alias for time-step)')
parser.add_argument('--T-train', default=None, type=int, help='random frame sampling count during training')
...
class NDADataset(torch.utils.data.Dataset):
    def __init__(self, base_dataset, H, W, T, split_by='number', train=True, use_nda=False):
        ...
        if use_nda and train:
            from augment import EventAugment
            self.event_augment = EventAugment((H, W))
        else:
    ...
    def __getitem__(self, idx):
        dict_events, label = self.base_dataset[idx]
        if self.event_augment is not None and random.random() < 0.5:
            ...
            events = torch.from_numpy(np.stack([x, y, t, p], axis=1))
        ...
        frames = integrate_events_by_fixed_frames_number(
            dict_events, self.split_by, self.T, self.H, self.W
        )
        return frames, label

def load_data(dataset, dataset_dir, distributed, T, use_tonic=False, use_nda=False):
    print("Loading data")
    st = time.time()
    if use_tonic:
        dataset_train, dataset_test, train_sampler, test_sampler = load_data_tonic(dataset, dataset_dir, distributed, T)
        print("Took", time.time() - st)
        return dataset_train, dataset_test, train_sampler, test_sampler
    if dataset == 'cifar10dvs':
        dataset_root = os.path.join(dataset_dir, 'CIFAR10DVS')
        if use_nda:
            origin_set = cifar10_dvs.CIFAR10DVS(root=dataset_root, data_type='event')
            dataset_train_raw, dataset_test_raw = split_to_train_test_set(0.9, origin_set, 10)
            H, W = cifar10_dvs.CIFAR10DVS.get_H_W()
            dataset_train = NDADataset(dataset_train_raw, H, W, T, 'number', train=True, use_nda=True)
            dataset_test = NDADataset(dataset_test_raw, H, W, T, 'number', train=False, use_nda=False)
        else:
            origin_set = cifar10_dvs.CIFAR10DVS(root=dataset_root, data_type='frame', frames_number=T, split_by='number')
        ...
        extract_dvs_dir = os.path.join(extract_root, 'DvsGesture')
        user_dvs_dir = os.path.join(dataset_dir, 'DvsGesture')
        if os.path.exists(user_dvs_dir) and not os.path.exists(extract_dvs_dir):
            os.makedirs(extract_root, exist_ok=True)
            if os.name == 'nt':
                import shutil
                shutil.copytree(user_dvs_dir, extract_dvs_dir)
            else:
                os.symlink(os.path.abspath(user_dvs_dir), extract_dvs_dir)
            print(f"[Auto-fix] Linked ./data/DvsGesture -> {extract_dvs_dir}")
        download_dvs_dir = os.path.join(dataset_root, 'download', 'DvsGesture')
        if os.path.exists(download_dvs_dir) and not os.path.exists(extract_dvs_dir):
            if not os.path.exists(extract_root):
                os.rename(os.path.join(dataset_root, 'download'), extract_root)
                print(f"[Auto-fix] Renamed download/ -> extract/ for local DVS128Gesture data")
            else:
                import shutil
                shutil.copytree(download_dvs_dir, extract_dvs_dir)
                print(f"[Auto-fix] Copied download/DvsGesture -> extract/DvsGesture")
        if os.path.exists(extract_dvs_dir) and not os.path.exists(events_np_root):
            print("[Auto-fix] Preprocessing aedat files to npz, this may take a few minutes...")
            os.makedirs(events_np_root, exist_ok=True)
            DVS128Gesture.create_events_np_files(extract_root, events_np_root)
            print("[Auto-fix] Done.")
        if use_nda:
            dataset_train_raw = DVS128Gesture(root=dataset_root, train=True, data_type='event')
            dataset_test_raw = DVS128Gesture(root=dataset_root, train=False, data_type='event')
            H, W = DVS128Gesture.get_H_W()
            dataset_train = NDADataset(dataset_train_raw, H, W, T, 'number', train=True, use_nda=True)
            dataset_test = NDADataset(dataset_test_raw, H, W, T, 'number', train=False, use_nda=False)
        else:
            dataset_train = DVS128Gesture(root=dataset_root, train=True, data_type='frame', frames_number=T, split_by='number')
            dataset_test = DVS128Gesture(root=dataset_root, train=False, data_type='frame', frames_number=T, split_by='number')
    print("Took", time.time() - st)
...
def main():
    ...
    if args.T is not None:
        args.time_step = args.T
    if not getattr(args, 'data_dir', None) and getattr(args, 'data_path', None):
        args.data_dir = args.data_path
    if not getattr(args, 'data_dir', None):
        args.data_dir = './data'
    
    is_event_dataset = args.dataset in ('cifar10dvs', 'dvsgesture')
    if is_event_dataset:
        args.prefetcher = False
        args.no_prefetcher = True
        if args.dataset == 'dvsgesture' and (args.num_classes is None or args.num_classes == 10):
            args.num_classes = 11
    ...
    if args.model == 'swformer_freq_aware':
        ...
        model = create_model(
            ...
            in_channels=in_channels, num_classes=args.num_classes, embed_dims=args.dim, drop_rate=0.,
    ...
    if is_event_dataset:
        dataset_train, dataset_eval, train_sampler, test_sampler = load_data(
            args.dataset, args.data_dir, args.distributed, args.time_step, use_tonic=args.use_tonic, use_nda=args.use_nda)
        if mixup_active:
            mixup_args = dict(
        ...
        loader_train = torch.utils.data.DataLoader(
            ...
            shuffle=(train_sampler is None or not isinstance(train_sampler, torch.utils.data.SequentialSampler)),
            sampler=train_sampler if isinstance(train_sampler, torch.utils.data.distributed.DistributedSampler) else None,
            num_workers=args.workers, drop_last=True, pin_memory=True)
        loader_eval = torch.utils.data.DataLoader(
            dataset=dataset_eval, batch_size=args.val_batch_size or args.batch_size,
            shuffle=False, sampler=test_sampler if isinstance(test_sampler, torch.utils.data.distributed.DistributedSampler) else None,
            num_workers=args.workers, drop_last=False, pin_memory=True)
    else:
        from timm.data import create_dataset, FastCollateMixup, AugMixDataset
    ...
    if args.local_rank == 0:
        ...
        if args.experiment:
            ...
            img_size_str = str(data_config['input_size'][-1]) if data_config else str(args.img_size)
            exp_name = '-'.join([
